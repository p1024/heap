...
class MinHeap {
	constructor(nodeList) {
		...
		MinHeap.makeHeap(this[Symbol.for('heap')]);
	}

	add(node) {
		let heap = this[Symbol.for('heap')];
		heap.push(node);
		MinHeap.fixedUp(heap, heap.length - 1);
	}
	...
	delete() {
		...
		heap.pop();
		MinHeap.fixedDown(heap, 0, heap.length);
		return deleteNode;
	}
	...
	static makeHeap(arr) {
		for(let i=Math.floor(arr.length/2) - 1; i>=0; i--) {
			MinHeap.fixedDown(arr, i, arr.length);
		}
	}
